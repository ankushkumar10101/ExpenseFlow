...
export default function Transactions() {
  ...
  useEffect(() => {
    ...
    if (typeFilter !== "All Types") {
      result = result.filter(
        (transaction) => transaction.type === typeFilter.toLowerCase()
      );
    }
  ...
  const handleQuickAdd = () => {
    navigate("/dashboard/add-transaction", { state: { from: "/transactions" } });
  };

  const handleEdit = (transaction) => {
    navigate("/dashboard/add-transaction", {
      state: { transaction, from: "/transactions" },
    });
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this transaction?")) {
      try {
        await api.delete(`/transactions/${id}`, { withCredentials: true });
        setTransactions(
      ...
      <div className="flex-grow-1 main-content main-content-shifted">
        <Container fluid className="p-4">
          <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 pt-3 gap-3">
            ...
            <Button
              className="btn btn-secondary-custom d-flex align-items-center justify-content-center gap-2 px-4 py-2 w-100 w-md-auto"
              onClick={handleQuickAdd}
              style={{
                backgroundColor: "#958fc4",
                border: "#6c757d",
                borderRadius: "10px",
              }}
              onMouseEnter={(e) =>
                (e.currentTarget.style.backgroundColor = "#221a54ff")
              }
              onMouseLeave={(e) =>
                (e.currentTarget.style.backgroundColor = "#958fc4")
              }
            >
              <MdAdd size={20} />
              Add Transaction
          ...
          <div className="bg-white p-3 rounded-4 shadow-sm mb-4 d-flex gap-3">
            <Dropdown onSelect={(selectedCategory) => setCategoryFilter(selectedCategory)}>
              <Dropdown.Toggle
                variant="light"
                className="rounded-pill border-0 bg-light px-3 text-muted"
                style={{
                  minWidth: "180px",
                  display: "flex",
            ...
            <Dropdown onSelect={(selectedType) => setTypeFilter(selectedType)}>
              <Dropdown.Toggle
                variant="light"
                className="rounded-pill border-0 bg-light px-3 text-muted"
                style={{
          ...
          <div className="bg-white rounded-4 shadow-sm overflow-hidden mb-4">
            <Table hover responsive className="mb-0 align-middle">
              <thead className="bg-light">
                <tr>
                  <th className="border-0 py-3 ps-4 text-muted fw-normal">
                    Title
                  </th>
                  <th className="border-0 py-3 text-muted fw-normal">
                    Category
                  </th>
                  ...
                  <th className="border-0 py-3 text-muted fw-normal text-end pe-4">
                    Actions
                  </th>
                </tr>
              </thead>
                ...
                {currentItems.map((transaction) => {
                  ...
                  return (
                    <tr key={transaction._id}>
                      ...
                      <td className="border-0 py-3">
                        <span
                          className={`badge fw-normal px-3 py-2 rounded-pill ${isIncome
                      ...
                      <td
                        className={`border-0 py-3 fw-bold ${isIncome ? "text-success" : "text-danger"
                          }`}
                      >
                      ...
                      <td className="border-0 py-3 text-end pe-4">
                        <div className="d-flex align-items-center justify-content-end gap-3 flex-nowrap">
                          ...
                          >
                            <div
                              className="rounded-circle border border-danger d-flex align-items-center justify-content-center"
                              style={{ width: "32px", height: "32px" }}
                            >
                              <MdDelete size={16} />
                            </div>
